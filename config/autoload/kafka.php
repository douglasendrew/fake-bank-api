<?php

declare(strict_types=1);
/**
 * This file is part of Hyperf.
 *
 * @link     https://www.hyperf.io
 * @document https://hyperf.wiki
 * @contact  group@hyperf.io
 * @license  https://github.com/hyperf/hyperf/blob/master/LICENSE
 */
use Hyperf\Kafka\Constants\KafkaStrategy;
use longlang\phpkafka\Client\SwooleClient;
use longlang\phpkafka\Socket\SwooleSocket;
use longlang\phpkafka\Timer\SwooleTimer;

use function Hyperf\Support\env;

$brokers = env('KAFKA_BROKERS', '127.0.0.1:9092');
if (is_string($brokers)) {
    $brokers = array_values(array_filter(array_map('trim', explode(',', $brokers))));
}

return [
    'default' => [
        'enable' => (bool) env('KAFKA_ENABLE', true),
        'connect_timeout' => (float) env('KAFKA_CONNECT_TIMEOUT', -1),
        'send_timeout' => (float) env('KAFKA_SEND_TIMEOUT', -1),
        'recv_timeout' => (float) env('KAFKA_RECV_TIMEOUT', -1),
        'client_id' => (string) env('KAFKA_CLIENT_ID', 'fake_bank_api'),
        'max_write_attempts' => 3,
        'bootstrap_servers' => ! empty($brokers) ? $brokers : ['127.0.0.1:9092'],
        'acks' => (int) env('KAFKA_ACKS', -1),
        'producer_id' => -1,
        'producer_epoch' => -1,
        'produce_retry' => 3,
        'produce_retry_sleep' => 0.1,
        'partition_leader_epoch' => -1,
        'interval' => 0,
        'session_timeout' => 60,
        'rebalance_timeout' => 60,
        'replica_id' => -1,
        'rack_id' => '',
        'group_id' => (string) env('KAFKA_GROUP_ID', 'fake_bank_group'),
        'group_retry' => 5,
        'group_retry_sleep' => 1,
        'group_heartbeat' => 3,
        'offset_retry' => 5,
        'auto_create_topic' => true,
        'partition_assignment_strategy' => KafkaStrategy::RANGE_ASSIGNOR,
        'sasl' => [],
        'ssl' => [],
        'client' => SwooleClient::class,
        'socket' => SwooleSocket::class,
        'timer' => SwooleTimer::class,
        'consume_timeout' => 600,
        'exception_callback' => null,
    ],
];
